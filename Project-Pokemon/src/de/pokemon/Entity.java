...
public class Entity {
	...
	protected enum LastDir {
		DOWN, LEFT, UP, RIGHT, NULL
	};

	protected LastDir lastDir = LastDir.DOWN;
	protected LastDir standDir; // Variable für Standanimation letzte
								// bewegungsrichtung
	...
	public void moveX(int tileX) {
		// Überprüft, ob Player bereits in Bewegung ist. Wenn ja wird Bewegung
		// bis zum nächsten Tile vortgesetzt
		if (isRunning) {
	...
	public void updateEntity(Input input, int steps) {
		...
		/*
		 * //Überprüft, ob Player bereits in Bewegung ist. Wenn ja wird bewegung
		 * bis zum nächsten Tile vortgesetzt if(isRunning){//TODO if(lastDir ==
		 * LastDir.DOWN){ if(posY % Core.tileSize == 0){ isRunning = false;
		 * tileY++; } else { posY += moveSpeed; } } else if(lastDir ==
		 * LastDir.UP){ if(posY % Core.tileSize == 0){ isRunning = false;
		 * tileY--; } else { posY -= moveSpeed; }
		 * 
		 * } else if (lastDir == LastDir.LEFT){ if(posX % Core.tileSize == 0){
		 * isRunning = false; tileX--; } else { posX -= moveSpeed; } } else
		 * if(lastDir == LastDir.RIGHT){ if(posX % Core.tileSize == 0){
		 * isRunning = false; tileX++; } else { posX += moveSpeed; } } } else
		 * if(input.isKeyDown(Input.KEY_A)){ lastDir = LastDir.LEFT; isRunning =
		 * true; isStanding = false; posX -= moveSpeed; }
		 * 
		 * else if(input.isKeyDown(Input.KEY_W)){ lastDir = LastDir.UP;
		 * isRunning = true; isStanding = false; posY -= moveSpeed; }
		 * 
		 * else if(input.isKeyDown(Input.KEY_D)){ lastDir = LastDir.RIGHT;
		 * isRunning = true; isStanding = false; posX += moveSpeed; }
		 * 
